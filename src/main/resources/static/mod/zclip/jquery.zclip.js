		...
		if (typeof c == "object" && !c.length) {
			...
			return this.each(function() {
				...
				if (e.is(":visible") && (typeof b.copy == "string" || a.isFunction(b.copy))) {
					...
					d.setHandCursor(b.setHandCursor);d.setCSSEffects(b.setCSSEffects);d.addEventListener("mouseOver", function(f) {
						...
						if (a.isFunction(b.afterCopy)) {
							...
							e.removeClass("hover");alert("Copied text to clipboard:\n\n " + g)
						}
						if (b.clickAfter) {
							e.trigger("click")
						}
					});d.glue(e[0], e.parent()[0]);a(window).bind("load resize", function() {
						d.reposition()
					})
				}
			})
			...
			if (typeof c == "string") {
				return this.each(function() {
					...
					var d = a("#" + e + ".zclip");
					if (c == "remove") {
						d.remove();f.removeClass("active hover")
					} else {
						if (c == "hide") {
							d.hide();f.removeClass("active hover")
						} else {
							if (c == "show") {
								d.show()
							}
...
var ZeroClipboard = {
	...
	Client : function(a) {
		...
		this.movieId = "ZeroClipboardMovie_" + this.id;ZeroClipboard.register(this.id, this);
		if (a) {
			this.glue(a)
		}
	}
...
ZeroClipboard.Client.prototype = {
	...
	glue : function(d, b, e) {
		...
		this.div.id = "zclip-" + this.movieId;$(this.domElement).data("zclipId", "zclip-" + this.movieId);
	...
	receiveEvent : function(d, f) {
		d = d.toString().toLowerCase().replace(/^on/, "");switch (d) {
		case "load":
